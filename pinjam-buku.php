...
if (isset($_POST['btn-save'])) {
	...
	if ($sisa == 0) {
		...
		echo "<meta http-equiv='refresh' content='0; url=?page=buku_anggota&msg=gagal'>";
	} else {
		$qt = $transaksi->t_transaksi($buku, $id_mhs, $mhs, $tgl_pinjam, $tgl_kembali, 'Pinjam', $ket, $id_buku);
		$qu = $transaksi->u_buku($id, 1);
		if ($qt && $qu) {
			echo "<script>alert('Transaksi Sukses');</script>";
			echo "<meta http-equiv='refresh' content='1; url=?page=buku_anggota&msg=success'>";
		} else {
			...
			<tr>
				...
				<td>
					<select required name="peminjam" class="form-control">
						...
						<option value="<?= $_SESSION['nim'] ?>"><?= $_SESSION['nama'] ?></option>
					</select> </td>
			</tr>
			<tr>
				<td>Tgl Peminjam</td>
