...
?><ul class="pagination"><?php
				...
				class buku extends perpus
				{
					...
					public function create($judul, $pengarang, $penerbit, $thn_terbit, $isbn, $jumlah_buku, $lokasi, $waktu, $bukunya, $covernya, $kategori)
					{
						try {
							$stmt = $this->conn->prepare('INSERT INTO tbl_buku(judul,pengarang,penerbit,thn_terbit,isbn,jumlah_buku,lokasi,tgl_input,bukunya,cover, id_kategori) VALUES(?,?,?,?,?,?,?,?,?,?,?)');
					...
					public function update($id, $judul, $pengarang, $penerbit, $thn_terbit, $isbn, $jumlah_buku, $lokasi, $bukunya)
					{
						try {
							$stmt = $this->conn->prepare("UPDATE tbl_buku SET judul=:judul, pengarang=:pengarang, penerbit=:penerbit, thn_terbit=:thn_terbit, isbn=:isbn, jumlah_buku=:jumlah_buku, lokasi=:lokasi, bukunya=:bukunya WHERE id=:id ");
							$stmt->bindparam(":bukunya", $bukunya);
				...
				class transaksi extends perpus
				{
					...
					public function t_transaksi($buku, $id_mhs, $mhs, $tgl_pinjam, $tgl_kembali, $status, $ket, $id_buku)
					{
						try {
							$stmt = $this->conn->prepare('INSERT INTO tbl_transaksi(judul,nim,nama,tgl_pinjam,tgl_kembali,status,ket,id_buku) VALUES(?,?,?,?,?,?,?,?)');
